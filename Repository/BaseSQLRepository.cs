    ...
    public class BaseSqlRepository
    {
        ...
        protected IList<T> GetEntitiesFromDatabase<T>(IDbCommand command, int attempt)
        {
            ...
            catch (SqlException ex)
            {
                ...
                {
                    throw new RepositoryException("Repository query execution error.", ex);
                }
            }
            finally
            {
        ...
        protected void ExecuteNonQueryChecked(IDbCommand command)
        {
            ...
            try
            {
                ...
                if (queryStatus == 0)
                {
                    throw new InvalidEntityException("The entity does not exist or it is obsolete.");
                }
            }
            finally
            {
        ...
        protected int ExecuteInt32Scalar(IDbCommand command)
        {
            ...
            {
                throw new RepositoryException("Repository query execution error.", e);
            }
            finally
            {
                CloseConnection(command);
